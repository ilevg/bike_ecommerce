...
function LoginAPI(props) {
  ...
  useEffect(() => {
    if (props.APIDetailsLogin.email && props.APIDetailsLogin.email.length > 0) {
      ...
      const url = "http://ilnytskyi.atwebpages.com/?rest_route=/simple-jwt-login/v1/auth";
      fetch(url, {
        method: "POST",
        body: formData,
      })
        ...
        .then((data) => {
          if (data["success"] == true) {
            ...
            setUrlToLogin(
              `http://ilnytskyi.atwebpages.com/?rest_route=/simple-jwt-login/v1/autologin&JWT=${data["data"]["jwt"]}&AUTH_KEY=Key123`
            );
          } else {
